...
data = read.csv("/Users/antonyjomon/Downloads/My Documents/stat11_ica/group27.csv")
x = data[,1] #Extracting the data required
i = 1:299
prob_tau = 1/298 #Our prior 

integranda <- function(sigsq,tau) {#Defining the first integrand 
  u<-1 #Setting our first value as 1
  for(i in 1:tau){#Defining the summation from 1 to tau 
    u = u * 1/(sqrt(2*pi*sigsq)) * exp(-1/2*(x[i]/sqrt(sigsq))^2) #Defined in the report
  }
  return(u)
}

integranda2 <- function(sigsq,tau) {#Second integrand 
  u<-1
  for(i in (tau:298)){#Defining the summation from tau to n-1 
    u = u * 1/(sqrt(2*pi*sigsq)) * exp(-1/2*(x[i]/sqrt(sigsq))^2) #Defined in the report
  }
  return(u)
}

integrandb <- function(sigsq){ #This is our p(sigma_1 ^2) function
  v = (1/(sigsq))^2 * exp(-1/sigsq)
  return(v)
}

integrand1 <- function(sigsq,tau){ #Defining the full LHS integrand as seen in the report
  int = integranda(sigsq,tau)*integrandb(sigsq)
  return(int)
}

integrand2 <- function(sigsq,tau){ #Defining the full RHS integrand as seen on the report
  int = integranda2(sigsq,tau)*integrandb(sigsq)
  return(int)
}

probs<- rep(0,298) #Empty list of all the probability values at that point
j<-1
while(j < length(probs)){#Looping through each possible value, we integrate the full likelihood function for each possible value 
  probs[j]=(integrate(integrand1,0,Inf,tau=j)$value)*(integrate(integrand2,0,Inf,tau=j)$value) 
  j = j+1
}
probs = (probs*prob_tau) / sum(probs) #Our posterior distribution
which.max(probs)#Finding the point with the highest likelihood of a change point

plot(i,x) #Plotting the data
abline(v=which.max(probs), col="blue") #Adding a line at the point we have determined
